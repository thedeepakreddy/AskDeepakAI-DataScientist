            ...
            <div>
              ...
              >
                <option value="auto" className="font-mono border-b border-slate-700 bg-emerald-900/20 text-emerald-300 font-black">
                  SUGGESTED DEFAULT: {edaFit.modelName}
                </option>
                ...
                <optgroup label="Ensemble & Tree Builders" className="text-indigo-300 font-black mt-2">
                  ...
                  <option value="lightgbm" className="font-mono text-slate-300 font-medium">LightGBM</option>
                  <option value="catboost" className="font-mono text-slate-300 font-medium">CatBoost</option>
                  <option value="decision_tree" className="font-mono text-slate-300 font-medium">Decision Tree CART</option>
                </optgroup>

                <optgroup label="Deep Learning" className="text-indigo-300 font-black">
                  <option value="mlp" className="font-mono text-slate-300 font-medium">Multi-Layer Perceptron (DNN / Artificial NN)</option>
                </optgroup>

                <optgroup label="Linear Methods" className="text-indigo-300 font-black">
                  <option value="linear" className="font-mono text-slate-300 font-medium">Linear / Logistic Regression Baseline</option>
                  <option value="svm" className="font-mono text-slate-300 font-medium">Support Vector Machine (SVC/SVR)</option>
                </optgroup>

                <optgroup label="Instances & Probabilities" className="text-indigo-300 font-black">
                  <option value="knn" className="font-mono text-slate-300 font-medium">K-Nearest Neighbors (KNN Distance)</option>
                  <option value="naive_bayes" className="font-mono text-slate-300 font-medium">Naive Bayes (Gaussian/Multinomial)</option>
                  <option value="kmeans" className="font-mono text-slate-300 font-medium">K-Means Segmenter (Unsupervised Cohorts)</option>
                </optgroup>
              </select>
      ...
      {orchestratedRes && (
        <div className="space-y-8 animate-fade-in">
          ...
          <div className="space-y-4">
            ...
            {activeRealm === 'comparison' && orchestratedRes.comparison && (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in text-xs">
                {/* Winner Card */}
                <div className="lg:col-span-1 space-y-6">
                  {orchestratedRes.comparison.filter(c => c.recommendationStatus === 'Winner').map(winner => (
                    <div key={winner.modelName} className="bg-emerald-900/10 backdrop-blur-md border border-emerald-500/30 p-6 rounded-2xl relative overflow-hidden flex flex-col hover:border-emerald-500/50 transition-colors shadow-[0_0_30px_rgba(16,185,129,0.05)]">
                      ...
                      <div className="relative z-10 space-y-4 flex-1">
                        <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-500/20 text-emerald-300 font-bold font-mono text-[10px] rounded-full border border-emerald-500/30 shadow-sm">
                          <CheckCircle className="w-3.5 h-3.5" /> RECOMMENDED MODEL
                        </div>
                        <div className="space-y-1 mt-6">
                          <h3 className="text-xl font-bold text-white font-display tracking-wide">{winner.modelName}</h3>
                          <p className="text-emerald-400/80 text-xs font-mono tracking-tight">{winner.methodType}</p>
                        </div>
                        
                        <div className="pt-6 mt-8 border-t border-emerald-500/20">
                          <p className="text-[10px] text-emerald-500/70 font-bold uppercase tracking-wider mb-1">Winning {winner.primaryMetric}</p>
                          <p className="text-3xl font-black text-emerald-400 font-mono tracking-tighter">
                            {(winner.metricValue * (winner.primaryMetric === 'Accuracy' || winner.primaryMetric === 'R2 Score' ? 100 : 1)).toFixed(2)}{winner.primaryMetric === 'Accuracy' || winner.primaryMetric === 'R2 Score' ? '%' : ''}
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="bg-slate-900/40 border border-slate-800 p-5 rounded-2xl flex flex-col justify-center">
                    <h4 className="text-[10px] font-bold text-slate-500 font-mono tracking-wider uppercase mb-4 text-center">Architectures Evaluated</h4>
                    <div className="space-y-3">
                       {Array.from(new Set(orchestratedRes.comparison.map(c => c.methodType))).map(type => (
                         <div key={type} className="flex items-center gap-3 bg-slate-950/50 p-2.5 rounded-lg border border-slate-850">
                           <div className="w-2 h-2 rounded-full bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.6)]"></div>
                           <span className="text-slate-300 font-mono text-[11px] font-semibold">{type}</span>
                         </div>
                       ))}
                    </div>
                  </div>
                ...
                {/* Score Chart & Table Replaced by Side-by-Side Model Comparison Grid */}
                <div className="lg:col-span-2 bg-slate-900/40 backdrop-blur-md border border-slate-800 p-6 rounded-2xl flex flex-col shadow-lg overflow-hidden">
                  <h4 className="text-[11px] font-bold text-slate-300 font-mono tracking-wider uppercase mb-6 flex justify-between items-center bg-slate-950/40 p-3 rounded-xl border border-slate-800">
                    <span className="flex items-center gap-2"><Sparkles className="w-4 h-4 text-indigo-400" /> Side-by-Side Algorithm Comparison</span>
                  ...
                  <div className="flex-1 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
                    <div className="flex gap-4 h-full min-w-max">
                      {[...orchestratedRes.comparison].sort((a, b) => b.metricValue - a.metricValue).map((c, idx) => {
                        const isWinner = c.recommendationStatus === 'Winner';
                        return (
                          <div key={c.modelName} className={`w-[260px] rounded-xl border p-5 flex flex-col justify-between shrink-0 relative overflow-hidden transition-all duration-300 ${isWinner ? 'bg-emerald-950/20 border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.05)]' : 'bg-slate-950/40 border-slate-800 hover:border-indigo-500/30'}`}>
                              ...
                              <div className="space-y-4">
                                <div>
                                  ...
                                  <p className={`text-2xl font-black font-mono tracking-tighter ${isWinner ? 'text-emerald-400' : 'text-indigo-300'}`}>
                                    {(c.metricValue * (c.primaryMetric === 'Accuracy' || c.primaryMetric === 'R2 Score' ? 100 : 1)).toFixed(2)}{c.primaryMetric === 'Accuracy' || c.primaryMetric === 'R2 Score' ? '%' : ''}
                                  </p>
                                </div>
                            ...
                            <div className={`mt-6 pt-4 border-t ${isWinner ? 'border-emerald-500/20' : 'border-slate-800/60'}`}>
                              <p className={`text-[10px] leading-relaxed font-medium ${isWinner ? 'text-emerald-500/80' : 'text-slate-500'}`}>
                                {isWinner ? 'Optimal balance of predictive accuracy and latency.' : 'Alternative configuration benchmarked for parity.'}
                              </p>
                            </div>
                          </div>
                        );
