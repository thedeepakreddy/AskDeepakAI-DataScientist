...
export default function EDAReport({ dataset, aiAnalysis, loadingAI, onTriggerAI }: EDAReportProps) {
  ...
  const numSummaries = calculateFullNumericSummaries(dataset.rows, dataset.columns);
  const missingDetails = mapAllMissingValues(dataset.rows, dataset.columns);
  const duplicates = detectDuplicateRecords(dataset.rows);
  const outlierDetails = findExtremeOutliers(dataset.rows, dataset.columns);
  const corrMatrix = computeCorrelationMatrix(dataset.rows, dataset.columns);
  ...
  const histogramData = selectedNumCol && activeColValues.length > 0
    ? (() => {
        const min = Math.min(...activeColValues);
        const max = Math.max(...activeColValues);
        const range = max - min;
        ...
        <div className="relative bg-[#0E1325]/85 backdrop-blur-md text-white p-6 sm:p-8 rounded-2xl border border-indigo-500/20 shadow-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 overflow-hidden">
          ...
          <div className="relative z-10 space-y-2.5">
            ...
            <h3 className="text-base sm:text-lg font-bold tracking-tight flex items-center gap-2 mt-1 font-sans">
              <Sparkles className="w-5 h-5 text-indigo-400 animate-pulse" /> Gemini-AI Strategic Insights
            </h3>
            <p className="text-xs text-slate-300 max-w-xl leading-relaxed font-sans">
              Activate automated AI diagnostics. Analyzes column patterns, profiles key business metrics, and offers high-accuracy target predictor recommendations using Gemini.
            </p>
          </div>
          <button
            onClick={onTriggerAI}
                  ...
                  <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-slate-950/30 p-4 rounded-xl border border-slate-850 shadow-inner">
                      ...
                      <div className="flex justify-between items-center text-[9px] font-mono text-slate-500 mt-2 px-2.5 border-t border-slate-850 pt-1.5">
                        <span>MIN Bounds: {Math.min(...activeColValues).toLocaleString()}</span>
                        <span>Normalized Continuous Coordinates</span>
                        <span>MAX Bounds: {Math.max(...activeColValues).toLocaleString()}</span>
                      </div>
                    </div>
...
function FeatureRelationshipScatter({ dataset, numericCols }: FeatureRelationshipScatterProps) {
  ...
  const minX = dataPoints.length > 0 ? Math.min(...dataPoints.map(p => p.x)) : 0;
  const maxX = dataPoints.length > 0 ? Math.max(...dataPoints.map(p => p.x)) : 100;
  const minY = dataPoints.length > 0 ? Math.min(...dataPoints.map(p => p.y)) : 0;
  const maxY = dataPoints.length > 0 ? Math.max(...dataPoints.map(p => p.y)) : 100;
