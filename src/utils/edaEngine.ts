...
export function detectDuplicateRecords(rows: any[]): DuplicateGroup[] {
  ...
  // For extreme datasets, limit duplicate search to the first 20,000 stringified elements to prevent browser crashes
  const sampleRows = rows.slice(0, 20000);
...
export function computeCorrelationMatrix(rows: any[], columns: DatasetColumn[]): {
  ...
  // Downsample to limit time complexity
  const STEP = Math.max(1, Math.floor(rows.length / 5000));
  const sampleRows = rows.filter((_, i) => i % STEP === 0);

  numericCols.forEach(colName => {
    columnData[colName] = sampleRows.map(r => {
      const val = r[colName];
